...
var paginator = (function () {
    ...
    */
    var paginatorPages = (function (pageTotal, changedClass, selectedPage) {
        ...
        for (var i = 0; i < pageTotal; i++) {
            ...
            if (i === numPages-1 && pageNumber != pageTotal) {
                var lastpage = i + 1;
                lastpage = lastpage + 1;
                newPages = newPages +
    ...
    var loadMorePages = (function () {
        $(document).on('click', '.jsNewPage', function () {
            ...
            for (var i = startFrom; i < maxPage + 1; i++) {
                ...
                if (i === startFrom + numPages-1 && i != maxPage) {
                    var lastpage = i + 1;
                    newPages = newPages +
                        '<li class="page-item jsPages jsNewPages"><span class="page-link jsNewPage" data-load="' + lastpage + '" >...</span></li>'
                    break;
            ...
            $('.jsPages').remove();
            $('.jspreviousPage').after(newPages);
            $('#__startPage').text($('.jsOldPage').attr('data-load'));
            $('#__endPage').text(parseInt($('.jsNewPage').attr('data-load'))-1);
        })
    })
    ...
    */
    var loadOldPages = (function () {
        $(document).on('click', '.jsOldPage', function () {
            ...
            if (startFrom > numPages+addPages) {
                var firstPage = startFrom - numPages;
                newPages = '<li class="page-item jsPages jsOldPages"><span class="page-link jsOldPage" data-load="' + firstPage + '" >...</span></li>';
            }

            else if (startFrom > numPages+1 && startFrom <= numPages+addPages) {
                var firstPage = totalPage;
                newPages = '<li class="page-item jsPages jsOldPages"><span class="page-link jsOldPage" data-load="' + firstPage + '" >...</span></li>';
            }

            for (var i = startFrom - numPages; i < startFrom; i++) {
                if (i > 0) {
    ...
    var pageSelection = (function (triggerFunc) {
        $(document).on('click', '.jsPage', function (e) {
            ...
            if (!$(this).hasClass('disabled')) {
                ...
                pageLimitControl();
                triggerFunc($('.js-paginator').data('pageSelected'));
            }
        })
    })
    ...
    */
    ...
    function initPaginator(data) {
        if(data.allPages != undefined && data.allPages > 0){
            numPages = data.allPages;
        }
